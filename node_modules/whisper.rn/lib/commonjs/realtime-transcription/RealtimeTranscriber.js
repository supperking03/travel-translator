...
class RealtimeTranscriber {
  ...
  constructor(dependencies) {
    ...
    this.options = {
      ...
      vadThrottleMs: options.vadThrottleMs ?? 1500,
      // Minimum time between VAD calls (ms)
  ...
  queueVADProcessing(slice) {
    ...
    if (this.options.vadSkipRatio > 0) {
      ...
      if (shouldSkip) {
        ...
        this.log(`Skipping VAD for slice ${slice.index} (skip ratio: ${this.options.vadSkipRatio})`);
        // Still queue for transcription if VAD would have approved
        this.queueSliceForTranscription(slice).catch(error => {
    ...
    if (this.isProcessingVAD) {
      ...
      this.log(`VAD busy, queued slice ${slice.index} (queue size: ${this.vadProcessingQueue.length})`);
      return;
    }
    if (timeSinceLastVad < this.options.vadThrottleMs) {
      // Too soon since last VAD, queue it
      this.vadProcessingQueue.push(slice);
      this.log(`VAD throttled, queued slice ${slice.index} (will process in ${this.options.vadThrottleMs - timeSinceLastVad}ms)`);
   ...
   */
  async processSliceVADThrottled(slice) {
    if (this.isProcessingVAD) {
  ...
  async processSliceVAD(slice) {
    try {
      ...
      if (this.callbacks.onBeginVad) {
        ...
        const shouldProcessVad = (await this.callbacks.onBeginVad({
  ...
  async processTranscription(item) {
    ...
    try {
      var _this$transcriptionRe, _this$callbacks$onTra, _this$callbacks5;
      // Build prompt from initial prompt and previous slices
  ...
  updateVadThrottleOptions(options) {
    ...
    this.log(`VAD throttle options updated: throttleMs=${this.options.vadThrottleMs}, skipRatio=${this.options.vadSkipRatio}`);
  }
